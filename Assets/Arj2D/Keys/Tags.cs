//This class is auto-generated do not modify
namespace k
{
	public static class Tags
	{
		public const string UNTAGGED = "Untagged";
		public const string RESPAWN = "Respawn";
		public const string FINISH = "Finish";
		public const string EDITOR_ONLY = "EditorOnly";
		public const string MAIN_CAMERA = "MainCamera";
		public const string PLAYER = "Player";
		public const string GAME_CONTROLLER = "GameController";
		public const string HIT = "Hit";
		public const string VENTANA = "Ventana";
		public const string PISO = "Piso";
		public const string ESCALERA = "Escalera";
		public const string ENEMY = "Enemy";
		public const string CABEZA = "Cabeza";
		public const string MANO = "Mano";
		public const string PROJECTILE = "projectile";
		public const string FX_TEMPORAIRE = "FxTemporaire";
		public const string OBJ_COLISION = "ObjColision";
		public const string NO_ELIMINA = "NoElimina";
	}
}